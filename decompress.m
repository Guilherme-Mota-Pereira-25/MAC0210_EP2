function decompress(compressedImg, method, k, h)
  ...
  elseif method == 2
    ...
    coefficients = zeros(size_c-1,size_c-1,3,4,4);
    h_matrix = [1 0 0 0; 1 h h^2 h^3; 0 1 0 0; 0 1 2*h 3*h^2];
    der = zeros(3,size_c,size_c,3);
    for c = [1:3]
      for i = [1:size_c]
	for j = [1:size_c]
	  if(i = 1)
	    der(1,1,j,c) = (compressedImg(2,j,c)-compressedImg(1,j,c))/h;
	  elseif(i = size_c)
	    der(1,size_c,j,c) = (compressedImg(size_c,j,c)-compressedImg(size_c-1,j,c))/h;
	  else
	    der(1,i,j,c) = (compressedImg(i+1,j,c)-compressedImg(i-1,j,c))/(2*h);
	  endif
	  
	  if(j = 1)
	    der(2,i,1,c) = (compressedImg(i,2,c)-compressedImg(i,1,c))/h;
	  elseif(j = size_c)
	    der(2,i,size_c,c) = (compressedImg(i,size_c,c)-compressedImg(i,size_c-1,c))/h;
	  else
	    der(2,i,j,c) = (compressedImg(i,j+1,c)-compressedImg(i,j-1,c))/(2*h);
	  endif
	endfor
      endfor
	...
	for j = [1:size_c]
	  if(i = 1)
	    der(3,1,j,c) = (der(2,2,j,c)-der(2,1,j,c))/h;
	  elseif(i = size_c)
	    der(3,size_c,j,c) = (der(2,size_c,j,c)-der(2,size_c-1,j,c))/h;
	  else
	    der(3,i,j,c) = (der(2,i+1,j,c)-der(2,i-1,j,c))/(2*h);
	  endif
	endfor
      endfor
	...
	for j = [1:size_c-1]
	      ...
	      pos_desc_j = j*(k+1)+y_inc;
	      cf = coefficients(i,j,c,:,:);
	      decompressedImg(pos_desc_i,pos_desc_j,c) = [1 x_inc x_inc^2 x_inc^3] * reshape(cf,4,4) * [1; y_inc; y_inc^2; y_inc^3];
            endfor
          endfor
	endfor
      endfor
	...
	for g_inc = [0:k]
          pos_desc_i = i*(k+1)+g_inc;
          decompressedImg(size(decompressedImg)(1),pos_desc_i,c) = [1 (k+1) (k+1)^2 (k+1)^3] * reshape(cf1,4,4) * [1; g_inc; g_inc^2; g_inc^3];
          decompressedImg(pos_desc_i,size(decompressedImg)(2),c) = [1 g_inc g_inc^2 g_inc^3] * reshape(cf2,4,4) * [1; (k+1); (k+1)^2; (k+1)^3];
	endfor
      endfor
      cff = coefficients(size_c-1,size_c-1,c,:,:);
      decompressedImg(size(decompressedImg(1)),size(decompressedImg)(2),c) = [1 (k+1) (k+1)^2 (k+1)^3] * reshape(cff,4,4) * [1; (k+1); (k+1)^2; (k+1)^3];
    endfor
  endif
  clear coefficients;
  imwrite(uint8(decompressedImg),"decompressedbi.png",'Compression',"none",'Quality',0);
endfunction
